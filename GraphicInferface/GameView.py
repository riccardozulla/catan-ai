...
import GraphicInferface.geomlib as geomlib
import Classes.CatanGraph as cg
import Classes.Board as Board
import GraphicInferface.PlaceCoordinates as pc
...
class GameView:
    def __init__(self, game, controller):
        ...
        self.robberImgPath = os.path.join(
            self.sourceFileDir, "imgs/robber.png")
        ...
        self.font_resourceLarge = pygame.font.SysFont(
            'tahoma', self.height//18)
        self.font_resourceMedium = pygame.font.SysFont(
            'tahoma', self.height//23)
        self.font_resourceSmaller = pygame.font.SysFont(
            'tahoma', self.height//28)
        self.font_resourceSmallest = pygame.font.SysFont(
            'tahoma', self.height//58, bold=True)
        ...
        for i in range(0, len(self.game.players)):
            self.points.append(self.font_resourceLarge.render(
                str(self.game.players[i].victoryPoints), False, self.playerColorDict[i+1]))
            self.pointsCards.append(self.font_resourceSmallest.render(
                "Vp cards: " + str(self.game.players[i].victoryPointsCards), False, self.playerColorDict[i+1]))
            self.woods.append(self.font_resourceSmallest.render(
                "Wood: " + str(self.game.players[i].resources["wood"]), False, self.playerColorDict[i+1]))
            self.sheeps.append(self.font_resourceSmallest.render(
                "Sheep: " + str(self.game.players[i].resources["sheep"]), False, self.playerColorDict[i+1]))
            self.crops.append(self.font_resourceSmallest.render(
                "Crop: " + str(self.game.players[i].resources["crop"]), False, self.playerColorDict[i+1]))
            self.irons.append(self.font_resourceSmallest.render(
                "Iron: " + str(self.game.players[i].resources["iron"]), False, self.playerColorDict[i+1]))
            self.clays.append(self.font_resourceSmallest.render(
                "Clay: " + str(self.game.players[i].resources["clay"]), False, self.playerColorDict[i+1]))
            self.knights.append(self.font_resourceSmallest.render(
                "Knights: " + str(self.game.players[i].usedKnights), False, self.playerColorDict[i+1]))

            self.monopolyCards.append(self.font_resourceSmallest.render(
                "MonopolyCards: " + str(self.game.players[i].monopolyCard), False, self.playerColorDict[i+1]))
            self.roadBuildingCards.append(self.font_resourceSmallest.render(
                "RoadBuildingCards: " + str(self.game.players[i].roadBuildingCard), False, self.playerColorDict[i+1]))
            self.yearOfPlentyCards.append(self.font_resourceSmallest.render(
                "YearOfPlentyCards: " + str(self.game.players[i].yearOfPlentyCard), False, self.playerColorDict[i+1]))
            self.playerType.append(self.font_resourceSmallest.render(
                "Type: " + str(self.game.players[i].type.name), False, self.playerColorDict[i+1]))

    def updateStats(self):
        for i in range(0, len(self.game.players)):
            self.points[i] = self.font_resourceLarge.render(
                str(self.game.players[i].victoryPoints), False, self.playerColorDict[i+1])
            self.pointsCards[i] = self.font_resourceSmallest.render(
                "Vp cards: " + str(self.game.players[i].victoryPointsCards), False, self.playerColorDict[i+1])
            self.woods[i] = self.font_resourceSmallest.render(
                "Wood: " + str(self.game.players[i].resources["wood"]), False, self.playerColorDict[i+1])
            self.sheeps[i] = self.font_resourceSmallest.render(
                "Sheep: " + str(self.game.players[i].resources["sheep"]), False, self.playerColorDict[i+1])
            self.crops[i] = self.font_resourceSmallest.render(
                "Crop: " + str(self.game.players[i].resources["crop"]), False, self.playerColorDict[i+1])
            self.irons[i] = self.font_resourceSmallest.render(
                "Iron: " + str(self.game.players[i].resources["iron"]), False, self.playerColorDict[i+1])
            self.clays[i] = self.font_resourceSmallest.render(
                "Clay: " + str(self.game.players[i].resources["clay"]), False, self.playerColorDict[i+1])
            self.knights[i] = (self.font_resourceSmallest.render(
                "Knights: " + str(self.game.players[i].usedKnights), False, self.playerColorDict[i+1]))
            self.monopolyCards[i] = self.font_resourceSmallest.render(
                "Monopoly: " + str(self.game.players[i].monopolyCard), False, self.playerColorDict[i+1])
            self.roadBuildingCards[i] = self.font_resourceSmallest.render(
                "RoadBuilding: " + str(self.game.players[i].roadBuildingCard), False, self.playerColorDict[i+1])
            self.yearOfPlentyCards[i] = self.font_resourceSmallest.render(
                "YearOfPlenty: " + str(self.game.players[i].yearOfPlentyCard), False, self.playerColorDict[i+1])
            self.playerType[i] = self.font_resourceSmallest.render(
                "Type: " + str(self.game.players[i].type.name), False, self.playerColorDict[i+1])

    def blit(self, player, x, y):
        playerBox = pygame.Rect(
            x-self.height//200, y-self.height//200, self.height//6.7, self.height//2.5)
        ...
        self.screen.blit(
            self.pointsCards[player.id-1], (x, y+self.height//18.2))
        ...
        self.screen.blit(
            self.monopolyCards[player.id-1], (x, y+self.height//4.2))
        self.screen.blit(
            self.roadBuildingCards[player.id-1], (x, y+self.height//3.6))
        self.screen.blit(
            self.yearOfPlentyCards[player.id-1], (x, y+self.height//3.3))
    ...
    def setupAndDisplayBoard(self, bg=True):
        ...
        if (bg):
            pygame.draw.rect(self.screen, pygame.Color(
                'cadetblue1'), (0, 0, self.gameWidth, self.height))
        hex_i = 0
    ...
    def drawGraphicTile(self, graphicTile):
        hexLayout = geomlib.Layout(geomlib.layout_pointy, geomlib.Point(
            self.height//12, self.height//12), geomlib.Point(self.gameWidth/2, self.height/2))
        ...
        pygame.draw.polygon(self.screen, pygame.Color(
            'black'), hexTileCorners, self.height//200)
        graphicTile.pixelCenter = geomlib.hex_to_pixel(
            hexLayout, graphicTile.hex)
        imgPath = os.path.join(
            self.sourceFileDir, self.imgDict[graphicTile.resource])
        image = pygame.image.load(imgPath).convert_alpha()
        mask = image.copy()
        mask = pygame.transform.scale(
            mask, (self.height//18.5, self.height//18.5))
        self.screen.blit(mask, (graphicTile.pixelCenter.x - self.height //
                         37, graphicTile.pixelCenter.y - self.height//17))
        self.drawNumberCircle(graphicTile)

    def drawNumberCircle(self, graphicTile):
        tileNumberText = self.font_resourceSmaller.render(
            str(graphicTile.number), False, pygame.Color("black"))
        if graphicTile.resource != 'desert':
            pygame.draw.circle(self.screen, pygame.Color("black"), (graphicTile.pixelCenter.x,
                               graphicTile.pixelCenter.y+(self.height//40)), self.height//37, self.gameWidth == 0)
            pygame.draw.circle(self.screen, pygame.Color("white"), (graphicTile.pixelCenter.x,
                               graphicTile.pixelCenter.y+(self.height//40)), self.height//43, self.gameWidth == 0)
            if (graphicTile.number >= 10):
                self.screen.blit(
                    tileNumberText, (graphicTile.pixelCenter.x-self.height//50, graphicTile.pixelCenter.y))
            else:
                self.screen.blit(tileNumberText, (graphicTile.pixelCenter.x -
                                 self.height//100, graphicTile.pixelCenter.y))
                ...
                if gtile.index == k:
                    for el in v:
                        if el not in alreadyFound:
                            ...
                            placeToAdd.setupCoords(
                                pc.getCoords(self, placeToAdd.index))
                            placeToAdd.setupSize(
    ...
    def updateScoreGNN(self, player):
        ...
        for i, evaluatePlayer in enumerate(player.game.players):
            normalizedScores[i] = Gnn.Gnn(
            ).evaluatePositionForPlayer(evaluatePlayer)
        ...
        scores = self.font_resourceMedium.render(
            scores_string, False, pygame.Color('grey'))  # self.playerColorDict[player.id])
        ...
        playersScores = pygame.Rect(
            self.gameWidth/2-self.height//4, 0, self.height//1.8, self.height//11)
        self.screen.fill(self.bgScoreColor, playersScores)
        self.screen.blit(
            scores, (self.gameWidth/2-self.height//4.3, self.height//100))
    ...
    def updateGameScreen(self, flag=False):
        ...
        if (2 <= self.game.nplayers):
            ...
            self.blit(self.game.players[1], self.gameWidth -
                      self.height//6.9, self.height//200)
        if (3 <= self.game.nplayers):
            self.blit(self.game.players[2], self.height //
                      200, self.height-self.height//2.6)
        if (4 <= self.game.nplayers):
            self.blit(self.game.players[3], self.gameWidth -
                      self.height//6.9, self.height-self.height//2.6)

        longestStreetBox = pygame.Rect(
            self.height//5.8, self.height-self.height//10, self.gameWidth * 0.1, self.height//10)
        self.screen.fill(self.bgScoreColor, longestStreetBox)
        font_longest_street = self.font_resourceSmaller.render(
            'LS: ' + str(self.game.longestStreetOwner.id), False, pygame.Color('white'))
        self.screen.blit(font_longest_street, (self.height //
                         5.7, self.height - self.height//10.5))
        font_longest_street_length = self.font_resourceSmaller.render(
            'len: ' + str(self.game.longestStreetLength), False, pygame.Color('white'))
        self.screen.blit(font_longest_street_length,
                         (self.height//5.7, self.height - self.height//22.2))

        largestArmyBox = pygame.Rect(self.gameWidth-self.height//3.7,
                                     self.height-self.height//20, self.height//10, self.height//20)
        self.screen.fill(self.bgScoreColor, largestArmyBox)
        font_largest_army = self.font_resourceSmaller.render(
            'LA: ' + str(self.game.largestArmyPlayer.id), False, pygame.Color('white'))
        self.screen.blit(font_largest_army, (self.gameWidth -
                         self.height//3.77, self.height - self.height//22.2))

        font_dice = self.font_resourceSmaller.render(
            str(self.game.dices[self.game.actualTurn-1]), False, pygame.Color('white'))
        diceRoll = pygame.Rect(self.height//5.8, 0,
                               self.height//20, self.height//20)
        ...
        if not flag:
            ...
            if len(self.stack.item_list) > 1 and len(undoStack) > 0 and self.stack.item_list[len(redoStack)+1]['button_element'] != None:
                pygame.draw.rect(self.stack.item_list[len(redoStack)+1]['button_element'].image, pygame.Color(
                    'red'), [0, 0, self.moveListWidth-self.height//140, self.height//35], 2)
    ...
    def drawStreet(self, edge, color):
        ...
        pygame.draw.line(self.screen, pygame.Color(
            "Black"), self.graphicPlaceList[startPos].coords, self.graphicPlaceList[endPos].coords, self.height//50)
        pygame.draw.line(
            self.screen, color, self.graphicPlaceList[startPos].coords, self.graphicPlaceList[endPos].coords, self.height//100)
    ...
    def checkAndDrawHarbors(self, graphicPlace):
        ...
        if graphicPlace.harbor is not None:
            harborPath = os.path.join(
                self.sourceFileDir, self.imgDict[graphicPlace.harbor])
            harborImg = pygame.transform.scale(pygame.image.load(
                harborPath).convert_alpha(), (self.height//20, self.height//20))
            ...
            if (idPlace in placesOnLeftUp):
                coords = (
                    graphicPlace.coords[0]-self.height//18, graphicPlace.coords[1]-self.height//18)
                self.screen.blit(harborImg, coords)
            elif (idPlace in placesOnLeftDown):
                coords = (
                    graphicPlace.coords[0]-self.height//18, graphicPlace.coords[1]+self.height//200)
                self.screen.blit(harborImg, coords)
            elif (idPlace in placesOnRightUp):
                coords = (
                    graphicPlace.coords[0]+self.height//200, graphicPlace.coords[1]-self.height//18)
                self.screen.blit(harborImg, coords)
            elif (idPlace in placesOnRightDown):
                coords = (graphicPlace.coords[0]+self.height //
                          200, graphicPlace.coords[1]+self.height//200)
                self.screen.blit(harborImg, coords)
            elif (idPlace in placesOnTop):
                coords = (
                    graphicPlace.coords[0]-self.height//40, graphicPlace.coords[1]-self.height//15)
                self.screen.blit(harborImg, coords)
            elif (idPlace in placesOnDown):
                coords = (
                    graphicPlace.coords[0]-self.height//40, graphicPlace.coords[1]+self.height//66)
                self.screen.blit(harborImg, coords)

    def drawRobber(self):
        robberImg = pygame.image.load(self.robberImgPath).convert_alpha()
        robberImg = pygame.transform.scale(
            robberImg, (self.height//15, self.height//15))
        robTile = Board.Board().robberTile
        for graphicTile in self.graphicTileList:
            if (graphicTile.index == robTile):
                robberCoords = (graphicTile.pixelCenter.x,
                                graphicTile.pixelCenter.y-self.height//33)
