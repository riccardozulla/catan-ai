...
class UseRobberCommand:
    ...
    def execute(self):
        self.actions.extend([SetRobberTile(self.tilePosition), StealResourceCommand(
            self.player, Board.Board().tiles[self.tilePosition])])
        for action in self.actions:
            action.execute()

        self.player.game.players[self.previousLastRobberUserId -
                                 1].lastRobberUser = False
...
class RemoveResourceToPlayer:
    ...
    def execute(self):
        assert self.player.resources[
            self.resource] > 0, f"Player {self.player.id} can't EXECUTE this action becouse it has not {self.resource}"
        self.player.resources[self.resource] -= 1
    ...
    def redo(self):
        assert self.player.resources[
            self.resource] > 0, f"Player {self.player.id} can't UNDO this action becouse it has not {self.resource}"
        self.player.resources[self.resource] -= 1
    ...
    def execute(self):
        if self.resource != "desert":
            if Bank.Bank().resources[self.resource] > 0:
                self.actions.append(AddResourceToPlayer(
                    self.player, self.resource))
...
class DiceProductionCommand:
    ...
    def execute(self):
        if (self.dice == 7):
            self.actions.append(SevenOnDicesCommand(
                self.game.currentTurnPlayer))
        else:
                    ...
                    for p in tile.associatedPlaces:
                        if (Board.Board().places[p].owner != 0):
                            if (Board.Board().places[p].isColony):
                                self.actions.append(BankGiveResourceCommand(
                                    self.game.players[Board.Board().places[p].owner-1], tile.resource))
...
class DefaultPassTurnCommand:
    player: Player
    # serve per prendere il secondo elemento di thing needed (che è un None)
    importantTemp: any = None

    def execute(self):
        self.player.game.actualTurn += 1
        self.player.game.currentTurnPlayer = self.player.game.players[
            self.player.game.actualTurn % self.player.game.nplayers]
...
class DiscardResourceCommand:
    ...
    def execute(self):
        self.playerSpendResource = PlayerSpendResourceCommand(
            self.player, self.resource)
...
class StealResourceCommand:
    ...
    def __post_init__(self):
        ...
        for place in self.tile.associatedPlaces:
            owner = self.player.game.players[Board.Board(
            ).places[place].owner-1]
        ...
        if len(playersInTile) > 0:
            self.chosenPlayer = playersInTile[random.randint(
                0, len(playersInTile)-1)]
    ...
    def execute(self):
        if self.chosenPlayer is not None and self.takenResource is not None:
            self.actions.append(AddResourceToPlayer(
                self.player, self.takenResource))
            self.actions.append(RemoveResourceToPlayer(
                self.chosenPlayer, self.takenResource))
        for action in self.actions:
            action.execute()
...
class TradeBankCommand:
    ...
    def execute(self):
        ...
        self.actions.extend([PlayerSpendResourceCommand(self.player, toGive)
...
class UseMonopolyCardCommand:
    ...
    def execute(self):
        ...
        for p in self.player.game.players:
            if p != self.player:
                self.actions.extend([RemoveResourceToPlayer(
                    p, self.resource) for _ in range(p.resources[self.resource])])
                self.actions.extend([AddResourceToPlayer(
                    self.player, self.resource) for _ in range(p.resources[self.resource])])
        for action in self.actions:
            action.execute()
...
class UseYearOfPlentyCardCommand:
    ...
    def execute(self):
        ...
        self.actions.append(BankGiveResourceCommand(
            self.player, self.resources[0]))
        self.actions.append(BankGiveResourceCommand(
            self.player, self.resources[1]))
        for action in self.actions:
            action.execute()
