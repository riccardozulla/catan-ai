...
class GameController:
        ...
        if (not self.speed and self.withGraphics):
            ...
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                if (event.ui_element == self.view.aiButton):
                    ...
                    self.view.stack.scroll_bar.set_scroll_from_start_percentage(
                        self.view.stack.scroll_bar.start_percentage+0.1)
    ...
    def playGameWithGraphic(self):
        ...
        reverseTurnOffSet = [
            *list(range(self.game.nplayers)), *list(reversed(range(self.game.nplayers)))]
        while True:
            if (self.game.actualTurn < self.game.nplayers*2):
                ...
                playerTurn = self.game.players[self.game.actualTurn %
                                               self.game.nplayers]
                self.decisionManager(playerTurn)
                if (playerTurn.victoryPoints >= 10):
                    self.saveToJson(playerTurn)
                    print(
                        f'Winner: {playerTurn.id}, Agent: {playerTurn.type}')
                    if (self.withGraphics):
                        pygame.quit()
