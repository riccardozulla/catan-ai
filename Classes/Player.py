...
class Player:
    def __init__(self, id, game, type: PlayerTypes = PlayerTypes.NOT_SET):
        ...
        # RESOURCES:
        self.resources = {"wood": 0, "clay": 0,
                          "crop": 0, "sheep": 0, "iron": 0}
    ...
    def printResources(self):
        print("Print resources of player:  ",
        ...
        for p in Board.Board().places:
            if (p.owner == 0):
                for p_adj in Board.Board().graph.listOfAdj[p.id]:
                    ...
                    # controlliamo che l'arco appartenga al giocatore, edges è un dictionary che prende in input l'edge e torna l'owner (il peso)
                    if (Board.Board().edges[edge] == self.id):
                        available = True
                        ...
                        if (available and Board.Board().places[p_adj].owner == 0 and self.nColonies < 5 and Board.Board().places[p_adj] not in possibleColonies):
                            possibleColonies.append(
                                Board.Board().places[p_adj])
    ...
    def evaluate(self, action):
        if (action == commands.DiscardResourceCommand):
            possibleCards = [r for r in self.resources.keys()
        ...
        # Yes they are the same method, but must be differentiated becouse of the count of knights.
        ...
        if (action == commands.UseRoadBuildingCardCommand):
            ...
            if len(self.ownedStreets) < 14:
                ...
                max1 = -1
                for edge in possibleEdges:
                    valutation = self.actionValue(
                        commands.PlaceFreeStreetCommand, edge)
                    if (max1 < valutation):
                        max1 = valutation
                        candidateEdge1 = edge
                toRet += max1
